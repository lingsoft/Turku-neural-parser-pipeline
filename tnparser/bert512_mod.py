...
def launch(args, q_in, q_out):
    if not args.merge:
        tokenizer = transformers.BertTokenizer.from_pretrained(
            args.vocabfile, local_files_only=True)
    ...
    for comments, sent in sentences:
        for row in sent:
            if "BERT512TRUNCATEDTOKEN_ORIG=" in row[MISC]:
                match = re.search(
                    r"\|?BERT512TRUNCATEDTOKEN_ORIG=(.*)=GIRO_NEKOT",
                    row[MISC])
                if not match:
        ...
        if comments == ["### TNPP MERGE INTO PREVIOUS"]:
            ...
            offset = int(
                current_sent[-1][ID])  #this much needs to be added everywhere
            for row in sent:
                parts = row[ID].split("-")
                parts = "-".join(
                    (str(int(p) + offset) for p in parts))  #add to everything
                row[ID] = parts
...
def split(sent, tokenizer, max_seq_len):
    ...
    for sent_row, token_tokenized in zip(sent, bert_tokenized):
        if len(token_tokenized) > max_seq_len or len(
                token_tokenized
        ) == 0 or token_tokenized == [
                '[UNK]'
        ]:  #we have a problem, the token itself is too long or weird in some manner
            if sent_row[MISC] == "_":
                sent_row[MISC] = "BERT512TRUNCATEDTOKEN_ORIG=" + sent_row[
                    FORM] + "=GIRO_NEKOT"
            else:
                sent_row[MISC] = sent_row[
                    MISC] + "|BERT512TRUNCATEDTOKEN_ORIG=" + sent_row[
                        FORM] + "=GIRO_NEKOT"
    ...
    for sent in new_sentences:
        if not sent:
            continue
        first_tok = int(
            sent[0][ID].split("-")[0]) - 1  #this much needs to be substracted
        for row in sent:
            parts = row[ID].split("-")
            parts = "-".join((str(int(p) - first_tok)
                              for p in parts))  #substract from everything
            row[ID] = parts
...
argparser.add_argument("--vocabfile",
                       help="Vocab file of the BERT model to use")
argparser.add_argument("--max-seq-len",
                       type=int,
                       default=400,
                       help="Max seq length to use, default %(default)d")
argparser.add_argument(
    "--merge",
    action="store_true",
    help=
    "Undo the transform. Without this parameter, it will run the transform, splitting sentences to max-seq-len pieces"
)
